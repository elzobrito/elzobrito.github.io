---
title: "The cache that speeds up AI must also separate users"
description: "vLLM's new cache_salt support for the Anthropic API shows why prefix reuse in shared environments needs an explicit boundary between users."
published: 2026-08-01
locale: en
translation: o-cache-que-acelera-a-ia-tambem-precisa-separar-usuarios
...
That optimization creates a question throughput cannot answer: who is allowed to share the same intermediate result? On August 1, [vLLM merged `cache_salt` support into its Anthropic Messages-compatible API](https://github.com/vllm-project/vllm/pull/49498). The patch is small, but it closes a meaningful gap between interfaces. OpenAI-compatible routes already offered explicit cache isolation; clients using the Anthropic format did not have the same control.
...
1. Map which routes actually expose the required isolation controls.
2. Define who shares a salt and where that value is stored.
3. Measure cache-hit rate, initial-read latency, and consumption per boundary.
4. Verify that client libraries preserve extended fields all the way to the server.
